...
> The frontend lives in [`blu3-client`](https://github.com/bluwwi/blu3) — this repo is the API + WebSocket server it talks to. If you're contributing to the client, you only need a running `blu3-server` instance and the three `NEXT_PUBLIC_*` env vars described in [Connecting the frontend](#connecting-the-frontend).
