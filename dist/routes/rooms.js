...
async function requireAuth(c, next) {
    ...
    try {
        const payload = await verify(header.slice(7), process.env.JWT_SECRET, "HS256"); // ← "HS256"
...
roomsRoute.post("/", requireAuth, async (c) => {
    ...
    let code = genCode();
    while ((await db.select().from(rooms).where(eq(rooms.code, code))).length > 0) {
        code = genCode();
    }
    const [room] = await db
        .insert(rooms)
...
roomsRoute.get("/:code", requireAuth, async (c) => {
    ...
    if (!room.isActive)
        return c.json({ error: "Room is closed" }, 410);
